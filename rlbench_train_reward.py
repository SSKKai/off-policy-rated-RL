...
class OPRRL(object):
    def __init__(self, sac_hyperparams, reward_hyperparams, rlb_env_config):
        ...
        set_seeds(sac_hyperparams.seed)
    ...
    def train(self):
        ...
        for i_episode in itertools.count(1):
            ...
            while not done:
                if self.start_steps_1 <= self.total_numsteps < self.start_steps_1 + self.sac_hyparams.start_steps:
                    action = self.env.randn_action()  # Sample random action
                else:
                    action = self.agent.select_action(state)  # Sample action from policy
        
                if len(self.agent_memory) > self.sac_hyparams.batch_size and self.total_numsteps > self.start_steps_1:
